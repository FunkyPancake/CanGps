...
{
    {
        std::vector<uint8_t> data = UbxMessage(UbxMessage::CFG, 0x8a, {
            0, 1, 0, 0x40, 0x52, 0x00, 0x01, 0, 0x07, 0x08, 0x00}).Serialize();
        _com->WriteBytes(data);
        auto a = _com->ReadBytes(10);
    }
    {
        std::vector<uint8_t> data = UbxMessage(UbxMessage::CFG, 0x8a, {
            0, 1, 0, 0x10,0x73,0x00,0x01,0x01}).Serialize();
        _com->WriteBytes(data);
        auto a = _com->ReadBytes(10);
    }
    {
        std::vector<uint8_t> data = UbxMessage(UbxMessage::CFG, 0x8a, {
            0, 1, 0, 0x10,0x74,0x00,0x01,0x01}).Serialize();
        _com->WriteBytes(data);
        auto a = _com->ReadBytes(10);
    }
    {
        std::vector<uint8_t> data = UbxMessage(UbxMessage::CFG, 0x8a, {
            0, 1, 0, 0x10,0x74,0x00,0x02,0x00}).Serialize();
        _com->WriteBytes(data);
        auto a = _com->ReadBytes(10);
    }
